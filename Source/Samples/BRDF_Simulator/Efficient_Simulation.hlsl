...
float3 resultColor(float3 N,float u, float v, float3 texPos, RWTexture2D currTexture, SamplerState currSampler) {
    ...
    //float4 color =
        currTexture[uint2(u * twidth, v * theight)] += 1.f ;
    ...
    //float3 colorRes =  (albedo / PI - normColor.r) * radiance * NdotL;
    //colorRes = colorRes / (colorRes + float3(1.0));
    //colorRes = pow(colorRes, float3(1.0 / 2.2));
        //normColor.r * radiance * albedo;

    return float(0.f);// normColor* radiance* albedo;;// normColor.r* radiance* albedo;;
}
...
float3 efficient_simulation(float3 texPos, float3 norm) {
    ...
    float phi = (atan2(R_.x, R_.z) - atan2(V_.x, V_.z) * M_1_PI + 0.5f);
    ...
    if (layerInd == 9) {
        color = resultColor(N, u, v, texPos, texture2d_0, gSampler_0);
        //color = float3(1.f);
    }
    else if (layerInd == 8 ) {
        color = resultColor(N, u, v, texPos, texture2d_1, gSampler_1);
        //color = float3(1.f);
    }
    else if (layerInd == 7) {
        color = resultColor(N, u, v, texPos, texture2d_2, gSampler_2);
        //color = float3(1.f);
    }
    else if (layerInd ==  6) {
        color = resultColor(N, u, v, texPos, texture2d_3, gSampler_3);
        //color = float3(1.f);
    }
    else if (layerInd == 5 ) {
        color = resultColor(N, u, v, texPos, texture2d_4, gSampler_4);
        //color = float3(1.f);
    }
    else if (layerInd == 4 ) {
        color = resultColor(N, u, v, texPos, texture2d_5, gSampler_5);
    }
    else if (layerInd == 3 ) {
        color = resultColor(N, u, v, texPos, texture2d_6, gSampler_6);
        //color = float3(1.f);
    }
    else if (layerInd == 2 ) {
        color = resultColor(N, u, v, texPos, texture2d_7, gSampler_7);
        //color = float3(1.f);
    }
    else if (layerInd == 1 ) {
        color = resultColor(N, u, v, texPos, texture2d_8, gSampler_8);
        //color = float3(1.f);
    }
    else if (layerInd == 0 ) {
        color = resultColor(N, u, v, texPos, texture2d_9, gSampler_9);
        //color = float3(1.f);
    }
    else {
        color = float3(0.f);
